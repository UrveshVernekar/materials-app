...
import { Search, ChevronLeft, ChevronRight, Download, Network, Loader2 } from "lucide-react";
...
export default function InventoryDistributionPage() {
  const [allItems, setAllItems] = useState<DistItem[]>([]);
  ...
  const handleExport = async () => {
    try {
      ...
      const data = filteredItems.map((item: any) => ({
        ...
        "Central Stock (GPC)": item.central_stock,
        "Branch Stock": item.branch_stock,
        "Total Stock": item.total_stock,
        "Balance Status": item.balance_status,
      }));
      ...
      XLSX.writeFile(wb, `Inventory_Distribution_${dateStr}.xlsx`);
    } catch (err) {
